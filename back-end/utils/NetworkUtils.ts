...
export const operateQuestion = async (req: express.Request, res: express.Response) => {
    let { token, status, id } = req.body;
    let Token = token.toString();
    let ID = parseInt(id);
...
export const handOnAnswer = async (req: express.Request, res: express.Response) => {
    let { token, answer } = req.body;
    let Token = token.toString();
    let Answer = JSON.parse(answer.toString());
    let userInfo: IStatus = await getUserInfo(Token)
    if (!userInfo.status) {
        OtherUtility.myLog(`更新失败<问卷>[token错误]`)
        res.send(userInfo)
    ...
    Answer.userID = userInfo.info?.data.ID;
    let res4Update = await StoreUtility.addAnswer(Answer)
    if (res4Update.status) {
        OtherUtility.myLog(`回复成功<回答>[]`)
    }
    else {
        OtherUtility.myLog(`回复失败<回答>[]`)
    }
    res.send(res4Update)
    return;
